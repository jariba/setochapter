...
model of this behavior. The two angles our architecture attack this issue 
are the following:
...
goals is directly derieved to the timelines and tokens from europa. 
such representation is rich enough to allow the exchange of state
...
The overall execution cycle of the agent is also abstracted as a
continuous planning/deliberation cycle between all the
reactors. At the agent interface level, each reactor provided
...
synchronization for a single reactor is illustrated in Figure 
\ref{fig:tick-exec}. Synchronization happens at every single 
tick and will interrupt planning allowing this reactor to identify
 its current state that can propagate through the reactor graph
 in order to ensure t aconsitent view of the state of the world 
...
This view of the reactor has two parallel tasks allow for long
...
described above to implement a reactor for which the sole purpose is to
deliberate as time advance. The design of this reactor takes cues from 
the IDEA architecture presented on \cite{mus02, mus06} in the sense taht both
planning and execution are based on a single unified model. The choice 
of \eu as the planning framework was directed by the fact that this 
framework provide a rich and highly configurable toolset to be able to 
infer around a rich temporal domain.
