...
\item \emph{Partitioning} the global planning problem into multiple
  ...
  \comment{$\pi \&  \lambda$ both defined for the first time} before
...
\begin{figure}[!htbp]
  ...
  \vskip-1cm
  \caption{\small Illustration of state information flow between the
    \texttt{Pilot} and the \texttt{Vehicle} reactors.  Observations
    transit in a bottom-up during synchronization and apply to the
    execution frontier $\tau$.  Goal information flow from the top to
    the bottom and concern the desired future values of state
    variables.\comment{ Fig should be cited in the text}}
  \label{fig:trex:flow}
\end{figure}
...
We first show how a reactor can track and identify state when it does
not have any compelling need to deliberate with no future goals. Even
...
framework. This flaw forces the reactor to identify its internal state
