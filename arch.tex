...
requests it produced that have not been observed \kcomment{replace
...
Both these deliberative \kcomment{``deliberative??''; something else
  or remove altogether perhaps?} functions have different scope and
...
an aid in identifying its {\em internal} timelines \kcomment{``use it
  as an aid to identify'' what though?} based on this plan. 
...
during the \kcomment{previous?} {\em deliberation} step. % in order to
...
new type of flaw which forces identification \kcomment{identification
...
not a straightforward sequence of unit decisions \kcomment{has a unit
  decision been explained in Europa?}. Since such a process is
...
planning scope \kcomment{ this last sentence should be reworked}. This
