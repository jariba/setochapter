...
\footnote{\kcomment{In such an event as the planner works towards goal
    satisfaction, the initial conditions leading to achieve that goal
    are no longer valid. The planner then tries to achieve a different
    goal which too has to be discarded similarly. And so on.}}  if the
internal state of the plan gets out of sync with the actual state of
the world.

\kcomment{A important consequence of this has been that} the planning
...
\item \emph{Partitioning} the global planning problem into multiple
  ...
  to the agent depending on where it is located in a reactor
  dependency graph such as shown in Fig. \ref{fig:agent}. A reactor
  closer to the hardware in the hierarchy, is expected to deliberate
  less and be highly responsive to observations from the world. This
  implies lower values of $\pi$ and $\lambda$ (see Section
  \ref{sec:arch:trex} below). \kcomment{In the current design, $\pi$
    and $\lambda$ are application dependent and fixed at design time.}
...
\rxe. However that is not the focus of this chapter; details are
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[scale=0.35]{figs/token-disptch.pdf}
  \caption{\small \kcomment{Dispatching a token occurs only when the request
    start time overlaps a window. \texttt{Goal$_1$} therefore gets
    dispatched from \texttt{reactor$_1$} to \texttt{reactor$_2$} but
    not \texttt{Goal$_2$} to avoid early commitment.}}
...
owner reactor of this state variable and avoiding early commitment.
\kcomment{Fig. \ref{fig:reactor-lookahead} illustrates an example
...
% may be used by other reactors.
Such a formalism also constrains reactor interaction to state
information exchange which comes in two forms as illustrated in
Fig. \ref{fig:trex:flow}:
...
\item {\em Observation on current state values}: This information
  ...
  advances. \kcomment{Observations can therefore be both sensor
    measurements as well as state information, passed up from other
    reactors. The strict rules of timeline ownership enable a clear
...
\begin{figure}[!htbp]
  ...
  tick cycle.}]{\label{fig:sync-cycle}\includegraphics[width=0.35\columnwidth]{figs/sync-tick-cycle}} 
  \qquad
  \subfloat[\small A \rx reactor's execution cycle: {\em Deliberation} is
    ...
    {\em tick} allowing integration of state information.]{\label{fig:tick-exec}\includegraphics[width=0.45\columnwidth]{figs/tick-cycle}}
  \caption{\small \kcomment{Detailed and abstracted views of
      synchronization and deliberation within one tick cycle.}}
...
\begin{figure}[b]
  ...
  with all its {\em internal} timelines while generating a \texttt{Failed} observation]{\includegraphics[width=0.35\columnwidth]{figs/AUV-fail0.pdf}
  \label{fig:fail0}}
  \qquad
...
\kcomment{search} tree is found with an alternate solution or the plan
...
synchronization and \kcomment{deliberation}. This allows these two
...
\begin{figure}[!b]
  ...
  synchronization.]{\label{fig:shop:exec0}\includegraphics[width=0.7\columnwidth]{figs/shoping_exec_t0}} \\
  ...
  observation was received by the reactor with the plan showed in
    \ref{fig:shop:exec0}.]{\label{fig:shop:exec1}\includegraphics[width=.7\columnwidth]{figs/shoping_exec_t1}}
...
with what was decided by the reactor. \kcomment{In the event of
  inconsistency, the (plan) expectations do not match the reality;
  then partial plan failure within the reactor is initiated.} This is
...
choices for the {\em current state flaw} \kcomment{as} shown in
...
\begin{figure}[!b]
  \centering
  \subfloat[\small Conflicting Observation]{\label{fig:shop:relax1}\includegraphics[width=0.7\columnwidth]{figs/shoping_exec_relax-1}}\\
  \subfloat[\small Relaxed Partial Plan]{\label{fig:shop:relax2}\includegraphics[width=0.7\columnwidth]{figs/shoping_exec_relax-2}}
