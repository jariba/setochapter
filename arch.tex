...
\subsubsection{Reactors failure handling in \rx}
...
produced the \texttt{Failed} observation as illustrated in in
Fig. \ref{fig:fail0}. Therefore, the
...
\item \texttt{Survey  controller} model allows him to integrate this
  ...
  this aspect of its current plan. In such case the agent structure
  remains as in Fig. \ref{fig:fail0} with no further impact.
\item \texttt{Survey controller} models is not able to handle this
  ...
  timeline (Fig. \ref{fig:fail1}).
\end{enumerate}
\begin{figure}[!htb]
  \centering
  \subfloat[\small When \texttt{Sampling} reactor fails to
  synchronize its removed from the graph and all its {\em internal}
  timelines produces the \texttt{Failed} observation]{\includegraphics[width=0.4\columnwidth]{figs/AUV-fail0.pdf}
  \label{fig:fail0}
  }
  \qquad
  \subfloat[\small In the worst case this \texttt{Failed} may imply
  other reactors hat depends on \texttt{Sampling} to  fail to
  synchronize implying a bottom-up destruction of faulty reactors.]{
    \includegraphics[width=0.4\columnwidth]{figs/AUV-fail1}
    \label{fig:fail1}
  }
  \caption{Impact of the \texttt{Sampling} reactor failure in the AUV
    agent from Fig. \ref{fig:agent}}
\end{figure}
