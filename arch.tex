...
level this execution latency is used to define the {\em dispatch
  window} of the same state variable.% : \comment{Needed to say that
  % early on but the flow of this paragraph is not very tight}
%\kcomment{Looks fine to me unless you want to clarify further.}
...
requests it produced that have not been executed yet.
% \kcomment{replace
%   ``observed'' with ``not incorporated by the owner reactor''?}
% yet
 This recall is sent to the reactor that received the goal(s) to
...
\item \texttt{step} that accepts new goals requested (if any) on the
  ...
  goal.
% to whichever reactor that is responsible for these. % \comment{By
...
Both these 
% deliberative \kcomment{``deliberative??''; something else
%   or remove altogether perhaps?} 
functions have different scope and temporal constraints. 
The \texttt{synchronize} call is considered as atomic and will 
focus only on state inference for the current tick by
...
an aid in resolving its {\em current state flaw} by using the plan
information on the exepected {\em internal} state evolution nearby the
execution frontier. 
% \kcomment{``use it
%   as an aid to identify'' what though?}
...
during previous {\em deliberation} steps. % in order to
% allow {\em synchronization} to identify its {\em internal} state.
Such a relaxation can be viewed as a backtracking % on the plan search
...
new type of flaw which requires to identify the value of all the {\em
  internal} state variables for the current tick.
% forces identification \kcomment{identification
%   of what? Probably need a little more explanation here.} of internal
% state of the reactor for the current tick. 
This flaw can be defined as:
...
\begin{figure}[!htbp]
  ...
  \caption{\small Illustration of synchronization flaws in a
    ...
    \kcomment{what line???} after the last token of each timeline
...
not a straightforward sequence of staright-forward decisions where 
the resolution can be easily identified by the solver as any other
option would leafd immediately to an inonsistency in the plan.
% \kcomment{has a unit decision been explained in Europa?}. 
Since such a process is required by the architecture at \emph{every}
tick for \emph{every} reactor the model needs to be designed carefully.
...
This filtering window complements the \rx {\em dispatch
  window} from Definition \ref{def:dispatch} which filters out at the
agent level those goals that are before the reactor execution latency
or too far into the future. The planning window reflect this filtering
of the agent by applying to tokens that are generated internally by the
reactor during its deliberation or the {\em goals} for which he
planner decided to postpone after the planning window -- probably
due to other tasks that needed completion beforehand.
% The planning window inside of a reactor
...
% planning scope \kcomment{ this last sentence should be reworked}. 
This allow the planner to focus on its planning horizon and report
decision on flawss thgat are beyond this scope on later ticks. As, 
time advance, these future flaws will eventuallty be part of the
planning window of the reactor and will be then resolved along with
potential new goals received from the agent.
% This helps the deliberation solver reduce the number of tokens to be
...
% will become active which a reactor can evaluate.
 This results in an apparent \emph{continuous planning} of the reactor. When there are no
...
% In that sense, and thanks to the carefull choice on our
...
% resonably small in comparison to the agent tick duration.
