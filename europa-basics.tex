...
from the generic theoretical frameworks that underlie \eu, to the
specific representation, modeling and algorithms implemented by
it. First, a quick introduction to Constraint Programming and
Constraint-Based Planning is provided.  Next, the specific
representation that \eu uses to perform Constraint-Based Planning is
presented.  After that comes an overview of \eu's architecture and how
it can be embedded in a planning application such as \rx. Finally,
\eu's modeling, inference and search mechanisms are covered in detail
so that the reader can understand how \rx can perform deliberative
planning and react to execution events by embedding and customizing
\eu.
