...
The next few sections will cover \eu in a bottom-up fashion, going from the 
generic theoretical frameworks that underlie \eu, to the specific representation, 
modeling and algorithms implemented by it. First, a quick introduction to 
Constraint Programming and Constraint-Based Planning is provided. 
Next, the specific representation that \eu uses to perform Constraint-Based Planning is presented.
After that comes an overview of \eu's architecture and how it can be embedded in a planning application
such as \rx. Finally,  \eu's modeling, inference and search mechanisms are covered in 
detail so that the reader can understand how \rx can perform deliberative planning and react to
execution events by embedding and customizing \eu.

\subsection{Automated Planning and \eu}
...
a solution found by \eu using a compact version of that formulation and a specialized search
procedure.
...
Programming or \textsf{CP}) is a discipline that provides a generic framework
...
\texttt{AC-3} algorithm \cite{mackworth77}, a popular implementation, improves
over the naive approach by ignoring constraints whose variables were
not affected since the last iteration. Fig.  \ref{fig:constraintnet} shows a set of constraints on 3 variables
($x$, $y$ and $z$) represented as a constraint network, and their domains before arc consistency is enforced.
After the AC-3 algorithm is executed, the domains for the 3 variables
would be pruned to x=$\{2$\}, y=$\{2$\}, z=$\{2$\}.
...
\texttt{IxTet} \cite{ghallab87,lemai04}, and \texttt{CAIP} (Constraint-Based
...
implementation of the \texttt{CAIP} framework. In \texttt{CAIP}, planning problems are
