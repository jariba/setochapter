...
The Constraint Reasoning Engine (CRE) manages variables, the domains
from which they can take values, and constraints that define
...
and efficiently plugged in. The Plan Database (PDB) manages object types
and objects, which are a mechanism to group variables to more naturally
model the real world in much the same way that object oriented
programming does. Also manages tokens, which are a mechanism to group
variables to represent temporally scoped state. Objects and tokens can
be used to model planning domains in a much more natural and extensible
way that a pure CSP approach can. The Rules Engine (RE) manages token
relationships in a planning model. The language modules (NDDL, ANML, and
...
and problem instances \comment{point advantages over pure programming
  API approaches, provide snippets}. Finally the API Layer (Fig.
\ref{fig:europa-api}) makes all the services available so that \eu can
be used to build applications.

\comment{Is Fig. \ref{fig:europa-api} necessary?}
...
\item Open Condition: an open condition is an inactive token. Inactive
  ...
  activated, rules may fire that create inactive slave tokens (TODO:
  point to more detailed decription above in Rules Engine section). Open
...
\comment{expand a little more on how heuristics can be implemented by applying ordering/filtering for chooseFlaw and makeDecisionPoint}
examples:
...
it is only one of many that could be implemented. TODO: give examples of
local search solvers for scheduling and CSP?
