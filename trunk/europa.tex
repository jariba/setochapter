...
\begin{enumerate}
	...
	\item \textbf{Variable Types and Instances} : All the object attributes are represented as variables in this model, for instance, it uses integer variables to represent the built-in temporal variables for each token (activity duration, start and end times), string variables to represent Location and Product names, and object reference variables to represent the structural relationships between different objects (for instance, each Agent has variable to keep track of its AgentLocation).
\end{enumerate}


\comment{explain rule definition below}
...
Now, having defined the Shopping agent domain that way, here is how a particular problem instance could be specified:
