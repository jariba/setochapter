...
encoding of temporal constraint relationships between tokens.
\kcomment{This para could do with a little embellishment especially
  since it is the only material that actually talks about the kinds of
  constraints and their origin.JB: I don't really know what else to
  say given the table above.}
...
\item[\textbf{Open Condition}:] an open condition is an
  ...
  \item[\textbf{Merging}:] A free activity \kcomment{this could be
      defined, since no def. for 'free' is provided} is merged with a
    matching activity already in the plan. An activity $a$ is said to
    match an activity $a?$ iff $a$ and $a?$ unify \cite{russelnorvig}
    and the temporal constraints involving $a$ are satisfied by
    $a?$. Thus, $a$ and $a?$ can be considered the same activity
  ...
  \item[\textbf{Activation}:] A new activity $a$ is introduced in the
    ...
    resulting from those domain rules are introduced as inactive
