...
Application Programming Interface (API) layers.  A user will typically
use the modeling facilities to inject a problem domain description,
and the API to extract and manipulate plans, and to obtain information
about plan flaws and constraint violations at any stage of the
planning process.
...
algorithms for new constraints can be easily and efficiently plugged
in.
...
The \textbf{Rules Engine} (RE) modularly adds the semantics of domain
rules that describe dependencies between tokens.
...
definitions in a much more natural and compact way. \texttt{NDDL} (New
Domain Description Language [pronounced ``noodle'']) \cite{NDDL} is
the main modeling language for \eu and will be described in detail in
the Modeling section below. ANML is a language under development that
...
problem domain in a concise way. The main characteristics that make
\texttt{NDDL} a powerful tool for describing problem domains are:

\begin{enumerate} 

\item Object Oriented: \texttt{NDDL} supports classes and inheritance
  in similar fashion to popular Object-Oriented languages like Java
  and C++. It also supports polymorphism for some of the planning
  components. Using object classes and instances is a time-tested
  approach to naturally describe problem domains \cite{rumbaugh}.
...
To understand how the most important elements of \texttt{NDDL} work,
we show how the Shopping Agent problem could be modeled:
...
\item \textbf{Object Types}: \texttt{NDDL} supports an object-oriented
  ...
  inheritance. We took advantage of that by making
  \textit{AgentLocation} inherit from \eu's built-in \textit{Timeline}
  class which doesn't allow predicates to overlap in time for the same
  object instance, in this way we can ensure that and agent can only
...
\item \textbf{Subgoals}: In \texttt{Agent::Go}, subgoals are used to
  ...
  subgoals (Allen operators are explained in more detail below), along
  with the temporal relationship each token representing a subgoal has
  with the the token representing the \texttt{Agent::Go} action. Also,
  \texttt{[condition, effect]} annotations can be used when defining a
  subgoal inside an action, \eu doesn't immediately associate any
  semantics with them, but they can be extremely useful for search
  modules when creating plans.

\end{enumerate}

Having that specification for the Shopping Agent domain, here is how a
particular problem instance could be stated:
...
In \eu, all the Variables and Constraints from a problem domain,
domain descriptions are kept inside the Constraint Reasoning Engine
(CRE).  The CRE provides a framework where any number of constraint
types can collaborate to perform inference and prune variable domains
using Bounds Propagation and Domain Reduction techniques. \eu uses a
Propagator pattern, in which a Propagator object keeps track of a set
of constraint instances of the same type and is responsible for
ensuring that whenever any variable changes or constraints are
introduced or dropped, all related variables are updated through
inference.
...
\kcomment{add europa's AC-3 algorithm here}
...
an efficient and complete algorithm for propagating an STN. \eu's
...
measured with real numbers, in terms of \eu's representation, resource
...
\begin{figure}
\centering
\includegraphics[scale=0.35]{figs/europa-resource-envelopes.png}
...
\item Open Condition: an open condition is an inactive token. Inactive
  ...
  is activated, rules may fire that create inactive slave tokens as described in section
  \comment{point to EUROPA Plan Representation section}.  Open Conditions can be resolved in three ways:
