...
\fcomment{Though it woulfd  be usefull to emphasize this ...}
The cost of modeling put aduring the intial design, for example,
should be balanced against the cost of redesigning
a new mission script eate every new deployment in more ``classical''
script-based controllers. 
...
resources for such experiments.\fcomment{This sentence does not make
  sence to me}
...
control loops where sensing, planning and acting are resulting from 
concurrent deliberating tasks within a formal framework. \fcomment{Got
  rid of sense-plan-act reference here. \rx plan/sense while sensing/planning.}
\texttt{T-REX} was designed after carefully
evaluating lessons learned from \texttt{IDEA} and owes substantially
to the notion of partitioned problem solving from that effort. However
this partitioning is systematic and methodologically principled where
we manage the information flow within the partitioned structure to
ensure consistency in order to direct the flow of goals and
