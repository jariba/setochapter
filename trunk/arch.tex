...
\item \emph{Partitioning} the global planning problem into multiple
  ...
  latency $\lambda$ 
  % \comment{$\pi \&  \lambda$ both defined for the first time} 
  before
  being able to produce plan to the agent depending on where it is
  located in a reactor dependency graph such as shown in
  Fig. \ref{fig:agent}.
...
solution that relaxes this static world assumption. \comment{Need to
  talk about IDEA and the fact that they had similar aspect even though
  they were never very clear nor well articulated in their papers}
...
assume that we have a reactor that has no future goal.Even
in this condition, \rx requires each reactor to produce at every tick
its {\em internal} state. For those reactors not owning {\em internal}
...
\begin{definition}
  \label{def:csf}
  A {\em current state flaw} indicate that one of the reactor {\em
    internal} timelines does not provide a fully defined state value
  for the execution frontier $\tau$. Such flaw can be due to one 
  of the 2 following reasons:
  \begin{itemize}
  \item The absence of a token in this timeline which overlaps the
    instant $\tau$. 
  \item An open decision between terminating the last past token for
    this timeline or pushing its end in the future.
  \end{itemize}
\end{definition}

The core idea bewteen this flaw is to ensure that at any single tick
the {\em synchronization} solver will identifies all the {\em
  internal} timelines for which the current state value is yet to be
identified. We handle their resolution based on following
proposition. 
\begin{proposition}
  \label{prop:csf:resolve}
  A {\em current state flaw} can  be resolved using one of the
  following choices which are eveluated accroding to this order:
\begin{enumerate}
...
\end{enumerate}
\end{proposition}
Intuitively the first choice enforces an intertail value asumption
where the reactor will tend to maintain its current as long as the
model and external observations allow it. The second choice 
favorise the adavnce of the reactor current plan if any; this direct
the {\em synchronization} decision toward assuming that its current
plan is properly executed. Finally if none of these two choices did
provide a valid solution -- meaning that both lead to an inconsittant
plan -- the reactor tries all the possible state values remaining for
this timeline until it finds one that allows it to identify its
state as required by \rx architecture. 
...
The steps 2 \& 3 are handled by the \eu solver from algorithm
...
assumption that the current state value of an
{\em internal} timeline does not depend on the future or more accurately
...
problematic when execution is taken into account especially as it puts
a strong emphasis between an evolving plan and an unknown future
...
% this relaxation gives already more felxibility for the reactor to
...
At the engine level careful choices were made in both the focus of the
...
previously -- was selected according to some simple assumptions that % takes
% benefit of a possibly assume the existing plan by assuming that 
...
\comment{that the past is monotonic?}. Such an assumption helps direct
...
The number of flaws between two steps \comment{between deliberation
  and synch.?} can evolve for the following reasons that are exogenous
to deliberation:
...
Such a scope allows filtering of tokens that are either necessarily
ending before $\tau$ or necessarily starting after, focusing the
planning problem on only those tokens that are within a ``reasonable''
