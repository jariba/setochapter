...
resolution choices for the {\em internal} state flaw presented in
Section \ref{sec:arch:sync} \fcomment{TODO: need to rework the
  presentation of thee choices so I can refer to them more directly --
  algorithm or pseudo theorem should do the trick}. Should such a flaw
be revealed during synchronization -- such that the current state of
...
inference when the next planning \texttt{step} resumes. Therefore they
\comment{what is the ``they''?} are not integral part of the plan
resolution (generating potentially new flaws for the planner or
contributing to resolve previously existing flaws). The planning
process is therefore perturbed internally and fully informed by how
the world is evolving even as it is planning.  This specific aspect
contrasts with other approaches we have which avoid perturbing the
planner during its search as it is not compatible with the ``off-line
planning'' assumption.
