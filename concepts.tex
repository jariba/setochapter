...
\item[\textbf{Tokens}] In first order logic, a predicate defines a
  relation between objects and properties \comment{use Genesereth citation?}.  In
...
\item \textit{Timelines}: Often objects in a domain must be described
  ...
  overlap may occur among them \cite{mus94}.  \comment{Maybe: add small
    example?}
...
\item[\textbf{Token State Model}] In \eu's representation, goals are
  ...
  \comment{Use Fig 4 and edit:: Add figure showing an AgentLocation timeline with an
